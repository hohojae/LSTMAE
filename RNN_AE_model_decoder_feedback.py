...
class LSTMAutoEnc(object):
    def __init__(self,
        ...
        self.sequence_length = sequence_length # 시퀀스렝스 = 노래길이
        self.learning_rate = 0.01

        # cell 정의
        self.enc_cell = tf.contrib.rnn.BasicLSTMCell(num_units=self.hidden_size, # hidden_size = 128
                                                     state_is_tuple=False)
        self.dec_cell = tf.contrib.rnn.BasicLSTMCell(num_units=self.hidden_size,
                                                     state_is_tuple=False)

        # Autoencoder는 인코더에 인풋과 디코더 인풋 스테이트를 플레이스홀더로 지정
        self.Enc_input = tf.placeholder(tf.int32, [None, self.sequence_length]) # N * 시퀀스렝스(송렝스)
        self.Dec_state = tf.placeholder(tf.float32, [self.batch_size, self.enc_cell.state_size]) # 배치사이즈*enc_cell상태사이즈

    def encoder(self, scopename): # scopename = pitch or duration
        self.enc_scope = "enc_{}".format(scopename)
        # one_hot : [sequence_length, input_size]
        self.x_one_hot = tf.one_hot(self.Enc_input, self.input_size) # Enc_input은 init할 때 정의됨.

        with tf.variable_scope(self.enc_scope): # enc_scope = "enc_pitch" or "enc_duration"

            # defining initial state
            self.initial_state = self.enc_cell.zero_state(self.batch_size, dtype=tf.float32) # cell의 초기상태 정의(zero_state로)
            enc_outputs, enc_state = tf.nn.dynamic_rnn(cell=self.enc_cell,                   # cell 구동!
